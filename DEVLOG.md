...
09-06-2026: testing pass: hooks, part 2 (useEvolutionChain, useTCGLibrary, useFavorites, useTeam) hook coverage complete

Closed out the fetch driven family. useEvolutionChain is a genuinely different shape from the last three: two sequential mocked calls (fetchSpecies → fetchEvolutionChain, the second can't even start until the first resolves since the evolution chain URL only exists inside the species response) followed by a parallel fan out (Promise.all over every stage's sprite). Deliberately didn't re-test buildEvolutionTree/collectEvolutionNames here those already have their own pure unit tests, same "trust the tested building block" rule as useAutocomplete not re testing debounce timing.

Two tests worth remembering the reasoning behind. First: proving the sprite fan out is actually parallel, not an accidentally sequential await in a loop that happens to produce the same final output. Couldn't assert on timing directly, so tracked an inFlight counter that increments on call and decrements on resolve a sequential version could never push that above 1, Promise.all should. Second: one stage's sprite fetch failing shouldn't take down the whole tree, just that one sprite (.catch(() => null) per-name, not wrapped around the whole Promise.all) tested by rejecting only the charmeleon fetch and asserting the tree structure is untouched while just that one sprite comes back null. That distinction — "one image missing" vs "whole feature broken" is exactly what'd get silently inverted if someone moved the .catch to the wrong scope during a refactor.

useTCGLibrary was the most structurally different: state lives in a useReducer (tcgLibraryReducer, already unit-tested in isolation) instead of scattered useStates, so the hook level tests only needed to prove the wiring, does the async side dispatch the right actions at the right moments. not re-litigate transition logic the reducer's own tests already cover. Two data sources funnel into that one reducer (named search vs. featured/shuffled), which meant mocking three API functions instead of one and deliberately leaving shuffleArray/sortTCGCards unmocked, since both already have their own pure-logic tests. Featured-view test checks the shape of the request (25 names, all drawn from the mocked full list) instead of the exact picks, so the test doesn't become secretly dependent on Math.random().

useFavorites/useTeam closed the hook list out, and correctly ended up tiny one test each. Both hooks are just a useContext read plus a guard clause; everything else they touch (add/remove, persistence, team capacity rules) already gets exercised through the components that actually consume them, so testing it a second time here via renderHook would've been duplicate coverage dressed up as new tests. The only genuinely untested line in either hook was the throw itself "fail loud with a clear message" instead of a null-pointer crash three calls later so that's the one thing each file checks, matching the exact error string. Also had to learn the vi.spyOn(console, "error").mockImplementation(() => {}) pattern here for the first time React logs to the console any time a component throws during render regardless of whether something catches it, so without suppressing it, a correctly passing test still prints scary looking noise into the test run.

09-07/11-2026 Component test coverage:
